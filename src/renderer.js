...
function buildHpLabelHtml(label, qrData, variant = "big") {
  const safeLabel = escapeHtml(String(label || "").trim().toUpperCase());
  ...
  const safeQr = buildQrUrl(qrData || label, 800);
  const logo = assetUrl("amazon-logistics-logo.png");
  const footerLine = footerText ? `<div class="hp-big-footer-code">${safeFooter}</div>` : "";

  return `<div class="hp-big-page">
    <div class="hp-big-card">
      <div class="hp-big-header"><div class="hp-big-title">${safeHeader}</div></div>
      <div class="hp-big-body">
        <img class="hp-big-qr" src="${safeQr}" alt="QR">
        ${footerLine}
        <img class="hp-big-logo" src="${logo}" alt="amazon logistics">
      </div>
    </div>
  </div>`;
}
...
function buildHpLabelStyles() {
  return `
    ...
    * { box-sizing: border-box; -webkit-print-color-adjust: exact !important; print-color-adjust: exact !important; }
    ...
    .hp-big-page { width: 100%; height: 100%; display: flex; align-items: center; justify-content: center; }
    .hp-big-card { width: 260mm; height: 150mm; border: 2.8mm solid #8b008b; border-radius: 12mm; overflow: hidden; background: #fff; display: grid; grid-template-rows: 42mm 1fr; }
    .hp-big-header { background: #8b008b; display: flex; align-items: center; justify-content: center; padding: 0 10mm; }
    .hp-big-title { color: #fff; font-size: 26mm; line-height: 1; font-weight: 900; letter-spacing: 1mm; white-space: nowrap; text-align: center; }
    .hp-big-body { position: relative; display: flex; flex-direction: column; align-items: center; justify-content: flex-start; padding-top: 12mm; background: #fff; }
    .hp-big-qr { width: 68mm; height: 68mm; object-fit: contain; image-rendering: pixelated; }
    .hp-big-footer-code { margin-top: 2mm; font-size: 10mm; line-height: 1; color: #000; font-weight: 900; text-align: center; }
    .hp-big-logo { position: absolute; bottom: 8mm; width: 42mm; max-height: 13mm; object-fit: contain; }
  `;
}
