...
^FO825,24^BQN,2,9^FDLA,${safeQrData}^FS
^PQ1,0,1,Y
^XZ
`;
  }

  const arrows = options.showArrows === false
    ? ""
    : `${buildDownArrowGraphic(42, 24)}${buildDownArrowGraphic(1100, 24)}`;
...
^PR4
^MD24
${arrows}^FO112,${font.y}^A0N,${font.h},${font.w}^FB${font.fieldWidth},1,0,C^FD${safeLabel}^FS
^FO805,24^BQN,2,9^FDLA,${safeQrData}^FS
^PQ1,0,1,Y
^XZ
`;
}
...
async function printEndcap(cluster, aisle, size) {
  ...
  const mode = size === "big" ? "big" : "small";
  const printer = size === "big" ? (getSettings().ovPrinter || getSettings().szPrinter) : getSettings().szPrinter;
  const zpl = buildZpl100x25Label(label, qrData, { mode, showArrows: false });
